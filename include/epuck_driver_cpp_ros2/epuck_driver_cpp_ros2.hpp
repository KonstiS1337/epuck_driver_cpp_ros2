...
class PiPuckRos2 : public rclcpp::Node {
	public:
		...
		~PiPuckRos2();
		bool initialize() {
			if(!initConnectionWithRobot()) return false;
			ioctl(fh, I2C_SLAVE, imu_addr);
			RCLCPP_INFO(this->get_logger(),"Starting Acc calib");
			calibrateAcc();
			RCLCPP_INFO(this->get_logger(),"Starting gyro calib");
			calibrateGyro();
			return initialized_;};
	private:

		int fh;
		unsigned char ros_to_epuck_[ACTUATORS_SIZE]; 
		unsigned char epuck_to_ros_[SENSORS_SIZE];
